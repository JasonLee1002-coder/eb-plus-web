...
const RULES = [
  ...
  },
  {
    code: "回本與存活率承諾",
    why:
      "對加盟主／買設備的人講「多久回本」「活下來的機率」。2026-08-29 新增：" +
      "加盟業締約前的資訊揭露有規範，誇大獲利可能觸及公平交易法第 21 條，" +
      "是本站最高風險的一類。全站掃出 15 處，其中一處還假稱「根據集團提供的數據」。" +
      "教讀者向對方索取依據的句子由 ALLOW 白名單放行。" +
      "「合約 3-10 年，太短可能回本困難」是合約年限不是承諾，已用否定詞排除",
    re: /(回本|回收期|投資回收|損益兩平)[^。]{0,16}?\d+(\.\d+)?\s*[-–~至]?\s*\d*(\.\d+)?\s*(個月|年|週)|\d+(\.\d+)?\s*[-–~至]?\s*\d*(\.\d+)?\s*(個月|年)[^。]{0,10}?(即可|可以|就能|便可)?\s*(回本(?!困難|不易|無望)|回收投資|損益兩平)|存活率[^。]{0,16}?\d+(\.\d+)?\s*%|\d+(\.\d+)?\s*%[^。]{0,10}?存活率|(回本期|回本週期|回收期|存活率)[^。]{0,14}?(更快|更短|縮短|高於|優於|普遍高|明顯高)/,
  },
  {
    code: "絕對化成效詞",
