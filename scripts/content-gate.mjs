...
const RULES = [
  ...
  },
  {
    code: "比較型百分比",
    why:
      "「比…高/低 N%」「高出 N%」這類對照式成效宣稱。2026-08-29 補：" +
      "原「成效百分比」規則要求句子帶提升/降低/節省等動詞，抓不到 " +
      "「食材成本比個體戶低 15–25%」「平均客單價高出 23%」。" +
      "這型句子當時在 8 個檔案裡活著，其中三處掛在東方美名下講採購成本",
    // 只抓「對照」語境：比…低 N% / 高出 N%。「至少 60%」「減少 30%」不在此規則
    re: /(比|較)[^。，、\s]{0,20}?(高|低|多|少|快|慢)\s*\**\s*\d+(\.\d+)?\s*[-–~至]?\s*\d*(\.\d+)?\s*%|(高出|高於|多出|少了|低了)\s*\**\s*\d+(\.\d+)?\s*[-–~至]?\s*\d*(\.\d+)?\s*%/,
  },
  {
    code: "絕對化成效詞",
  ...
  {
    code: "規模宣稱",
    why: "門市數／車隊數／年資等規模數字，只准用有送件依據且當場標明來源者",
    re: new RegExp(
...
console.error("  · 規模宣稱 → 只准用有送件依據的數字，且要在同一句標明來源（例：依科專計畫書送件資料）");
